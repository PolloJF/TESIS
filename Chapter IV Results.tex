...
\newp
With the geometry and characteristics of the coil defined, the associated electrical parameters are calculated. First, the coil's inductance is calculated using the formula for a toroidal coil:
...
\begin{itemize}
    ...
    \item $A$ is the cross-sectional area of the toroid in square meters ($m^2$).
    \item $l$ is the mean length of the magnetic path in meters (m).
\end{itemize}

Considering an approximate cross-sectional area  of $A = -m^2$ and an average magnetic path length of $l = - m$, a theoretical inductance of $L ≈ -H$ is obtained. However, when measuring the inductance of the constructed coil using an LCR meter, experimental values of $L_1 = 10.67 mH$ and $L_2 = 10.55 mH$ are obtained. This discrepancy can be attributed to factors such as the non-uniform distribution of the magnetic field, conductor losses, and tolerances in the coil's construction.
\newp

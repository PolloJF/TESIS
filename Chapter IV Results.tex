...
\begin{figure}[H]
    ...
    \includegraphics[scale=0.7]{img/Desfase_end_2_end_2.jpg}
        \caption{Phase shift}
...
\subsubsection{Ortogonality and Bandwidth}\label{subsec: ortogonality}
...
\subsubsection{Correction Error Code}\label{subsec: error_correction_code}
...
\subsection{Induction Pattern}
 The decaiment of the power available in the receiver coil as a function of the relative angle between coils was measure at surface level in a open field. The transmitter coil was fixed in a horizontal position, while the receiver coil was rotated in steps of 5$\degree$ 
...
As expected, the power received is maximum when the coils are perfectly aligned (0$\degree$) and decreases as the angle of missalignment increases. The decaiment of voltage induced in the receiver coil follows a cosine pattern, which is consistent with the theoretical model of angular missalignment \ref{equation_of_flux_density_angular_misalignment}. In this case the power delivered is proportional to the square of the voltage, so the decaiment of power follows a cosine squared pattern. At 45$\degree$, the power received is approximately 50\% of the maximum value (-3 dB).
...
\newp
Here is a table of the Symbol Error Rate (SER) obtained in each test considering both preamble models and the aplication of Reed Solomon error correction code with 20 parity symbols:

\begin{table}[H]
    \centering
    \begin{tabular}{|c|c|c|c|c|}
    \hline
    \textbf{Preamble Model} & \textbf{RX Location} & \textbf{SNR (dB)} &    \textbf{SER without RS} & \textbf{SER with RS} \\ \hline 

...
\newp
The communication system is defined primarily by the magnetic coupling principle between two resonant coils, one used as a transmitter and the other as a receiver. Also, the system has to be bidirectional, so both coils can work as a transmitter or a receiver depending on the communication direction.
\subsection{Design}
...
As shown in the diagram, we can start from left to right with the generation of a text message, which is codified and modulated digitally in a computer. Then the digital signal is converted to analog using a DAC. This signal is amplified using a class D audio amplifier to drive the resonant coil. The flux of current at the coil generates a variable magnetic field, as equation \ref{equation_of_ampere_maxwell} describes in section 2. This variable magnetic field, which is centered at a pretty low frequency (VLF band of radio), propagates through the environment, penetrating rock, soil, and solid material and reaching the receiving coil.
\newp
The magnetic flux that passes perpendicularly through the coil structure can induce a voltage in the terminals of the receiving coil, according to Faraday's law of induction \ref{equation_of_magnetic_induction}. This signal can be digitized using an ADC, demodulated, decoded, and corrected in a computer to recover the original text message.
\newp
...
\newp
With the geometry and characteristics of the coil defined, the associated electrical parameters are calculated. First, the coil's inductance is calculated using the quick formula for circular coils \ref{eq:coil_inductance},
where:
...
Thus, the capacitance required to tune the coil to approximately 3.2 kHz results in C ≈ 0.237 μF for L1 and C ≈ 0.240 μF for L2. 0.470 μF ceramic capacitors in series were selected to achieve the desired value, accounting for component tolerances. A diagram of the constructed coil is shown below in figure \ref{fig:coil}.
...
Knowing the values of inductance and capacitance of the resonant coil, it's possible to characterize the transfer function of the system. Considering the schematic of the induced coupling between two symmetrical resonant coils shown in figure \ref{fig:inductive-coupling-schematic}. We can determine the transfer function of the system using the equation described in Section 2.3.1.
...
For the Python simulation, we obtain a similar result shown in Figure \ref{fig:transfer-function-py}.
...
\begin{figure}[H]
    ...
    \caption{Transfer function of the resonant coils separately.}
    \label{fig:transfer-function}
\end{figure}

The phase difference of the complete system (end-to-end) measured at a distance of 2 meters between the coils is shown below.

\begin{figure}[H]
    \centering
    
    \includegraphics[scale=0.3]{img/Desfase_end_2_end2.png}
        \caption{Phase shift}
    \label{fig:sub2-transfer-function-system}
\end{figure}

The power relation of the complete system (end-to-end) measured at a distance of 2 meters between the coils is shown below.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{img/Relacion_de_amp_end_2_end.jpg}
    \caption{Amplitude relation of the complete system.}
    \label{fig:sub1-transfer-function-system}
\end{figure}



If we review the power relation graph in Figure \ref{fig:sub1-transfer-function-system}, we can observe that the maximum power transfer occurs at a frequency of approximately 3.2 kHz, specifically at 3150 Hz. At this point, we have an approximately analog bandwidth of ~450 Hz, considering a decrement of 3 dB from the center frequency. Here it's a zoom of this graph in figure \ref{fig:zoom-transfer-function-system}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{img/bw.png}
    \caption{Bandwidth available for end-to-end system at 2 meters distance.}
    \label{fig:zoom-transfer-function-system}

\end{figure}

If we consider this geometry and values of the coil, we can estimate the magnetic field generated at the center of the coil using the formula decribed in section 2 \ref{equation_of_magnetic_field} for the magnetic field at the center of a circular loop:

%ecuacion y calculos pendientes

\subsection{Amplifier and GLI}

The amplifier chosen for the application corresponds to a class D Power Amplifier Board XH-M544  based on the Texas Instruments chips TPA3116D2 and NE5532. This amplifier is capable of delivering a maximum power of 150W Mono, considering a voltage of 26 V.
...
\begin{figure}[H]
    ...
    \caption{Class D Amplifier TPA3116D2.}
    \label{fig:amplifier}
\end{figure}
...
The switching circuit is essential for enabling bidirectional communication in the TTE system. For this prototype, this function is supplied by a positional selector, which means it is manually supplied.
\newp
Here is a schematic diagram of the positional selector used in MAGIC.
\begin{figure}[H]
    \centering
...
These components were packaged in addition to a switch and DC connectors in a 3D printed enclosure designed and printed for this specific purpose. Here is a figure of the result:

\begin{figure}
    \centering
    \includegraphics[width=0.5\textwidth]{img/Ensamblaje_bat.jpg}
    \caption{3D printed enclosure for battery.}
    \label{fig:enclosure}
\end{figure}
%incluir también imagenes reales de la construcción

The rest of the components that are not marked in the schematic diagram on figure \ref{fig:system-schematic} are also enclosed in another 3D printed box, denominated core module of MAGIC, that contains the amplifier, GLI, positional selector, and a voltimeter/amperimeter monitor. This module is connected to the coil through a typical DC connector and the audio interface using standard audio cables with 3.5 mm jacks.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{img/diagrams/Core Module.png}
...
\subsection{Design}
The software component of the TTE communication system is responsible for coding, modulating, and generating the message signal at the Transmitter, and filtering, detecting, demodulating, and decoding the received signals at the receiver. The software is developed in Python, leveraging libraries such as NumPy and SciPy for signal processing tasks.
\newp
The modulation scheme selected for the system is non-coherent Frequency Shift Keying (FSK). This choice is motivated by its robustness to phase variations. As it was explained in Chapter II, the composition of the media can produce significant phase shifts and frequency offsets that can affect the performance of coherent modulation schemes. The magnetic coupling scheme is intrinsically dispersive at resonant frequency, and that goes even worse when the constant of coupling $k$ is reduced by the distance and other attenuation sources.
...
The function of MAGIC is to generate a text message link, so it is necessary to generate, in principle, any letter of the alphabet and all the numbers. If we consider also other characters like punctuation marks and special symbols, the quantity of different characters to code increases significantly. In that way, we can use the ASCII (American Standard Code for Information Interchange) coding scheme, which has 128 different characters.
\newp
To encode characters using FSK modulation, we can assign a unique frequency to each character. However, this means that we need at least 128 different frequencies in our constellation, and we also have to consider a minimum spectral separation between tones to achieve the orthogonality condition. This requirement can lead to a very wide bandwidth, and we want to concentrate the constellation at the center frequency of the resonant coil in order to favor the conditions of communication.
\newp
...
\newp
For example, the character 'A' has an ASCII value of 65; we can obtain two hexadecimal digits from this value: 4 and 1, which correspond to the integer division and the modulus of the original value by 16, then we transform the hexadecimal digits to their decimal form and map to the corresponding FSK frequency. In this case, 4 maps to $f4$ and 1 maps to $f1$.

\subsubsection{Orthogonality and Bandwidth}\label{subsec: ortogonality}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{img/ser_len.jpg}
...
\newp
At the beginning, we define the 16 frequencies around the center frequency of the system that corresponds approximately to 3.2 kHz. Considering a minimum separation of 4 Hz, that's because we want to leave a spectral channel between symbols, considering the FFT resolution at the demodulator. With this criterion, we can use a minimum bandwidth of 60 Hz to allocate the 16 frequencies.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{img/symfreq.png}
    \caption{Frequency constellation for 16-FSK modulation.}
    \label{fig:frequency-constellation}
\end{figure}

In figure \ref{fig:frequency-constellation} we can observe the defined frequency constellation for the 16-FSK modulation scheme and a measure o  the spectrum of noise in one of the locations where the system was tested in Cerro Calán. The noise, which is also filtered by a digital filter, shows an electrical grid harmonic at 3150 Hz, which is close to the first symbol frequency.
\newp
After doing several measurements of the spectral distribution of Noise in different locations, it was observed that the odd harmonics of the electrical grid (i.e., 3150 Hz, 3250 Hz, etc.) were really prominent, while the even harmonics were less significant. Here are some examples of noise spectra measured in different locations.
...
Although the even electric grid harmonics are strangely less significant, to avoid potential interference, the frequency constellation was designed to avoid these harmonics as well. Therefore, the selected frequencies for the FSK modulation scheme were modified, defining flagged bands without symbols around both odd and even harmonics. Here is the final frequency constellation used in the system:
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{img/100m 200m and noise/newc.png}
...
\newp
In comparison, for example, models based on PSK or QAM are more sensitive to phase noise and frequency offsets. FSK modulation allows the receiver to detect the transmitted symbols based on energy detection in specific frequency bands, without requiring precise phase synchronization. This characteristic makes non-coherent FSK particularly suitable for TTE communication systems, where the channel conditions can be highly variable and unpredictable.
Here is a diagram of the receiver algorithm \ref{fig:fsk-mod-demod}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{img/model_rec.jpg}
    \caption{Block diagram of the non-coherent FSK modulation and demodulation process.}
    \label{fig:fsk-mod-demod}
\end{figure}

At the beginning, the digital signal is filtered and then passes to a Window of length N, which is an important number; the dimensions of this Window determine the cost of the main process of detection, which means the IQ correlation. The bigger the Window is, the more calculations occur in each earing cycle.
\newp
The data present in the window is processed using the correlation by parts algorithm, which is explained in the next section. The output of this process is an array of correlations, in which we can obtain the maximum. If this peak of correlation is above a certain threshold, which is previously determined by the place conditions, we save that value and its position. If not, we discard the time-older data chunk to put in a new one. That's why the window is also a queue structure, specifically a \textit{deque} from the collections library of Python.
\newp
If we have detected a peak of correlation, we repeat the process saving the peaks and their position in the next earing cycles, when the peak is identified as a global one, achieving a increase and then a decrease in its value, we proceed to save the data that temporally is next from preamble, that's is, counting samples from the position of the peak plus the size of the preamble. Once the message position is known, it proceeds to save the following symbols, knowing beforehand the size of the total message.
\newp
The demodulation process is performed by calculating the FFT of each symbol received and determining which frequency has the highest energy. This frequency is then mapped back to the corresponding symbol in the FSK constellation. Finally, the decoded symbols are converted back into characters using the inverse of the encoding scheme used at the transmitter. Also, parity symbols are checked to correct possible errors in the received message.

\subsubsection{Digital Filter}

The digital filter used in the receiver is a bandpass filter designed to isolate the frequency band of interest around the FSK symbol frequencies while attenuating noise and interference outside this band. The filter is implemented using a Butterworth design due to its maximally flat frequency response in the passband, which helps preserve the integrity of the received signal. For this design, we use the scipy function \textit{buttord} to define the performance and frequency response of the filter and \textit{butter} to create the filter coefficients. The specifications are:
...
The filter is applied with the function \textit{filtfilt} from the scipy library, which performs zero-phase filtering by processing the input signal in both the forward and reverse directions. This approach helps to eliminate phase distortion introduced by the filter, which is particularly important in communication systems where phase information can be critical for accurate symbol detection.%cita pendiente
\newp
Here is the frequency response of the designed bandpass filter:

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{img/filter.jpg}
...
Here we have several main design parameters: an optimal length of preamble, its structure, and optimal use of our computation resources, because the bigger the length of correlation inputs, the more calculations to do in each earning time. %Improve redaction
The size of the window is key to the times that the function \textit{Correlation by parts} is called. 
...
The correlation formula (\ref{eq:Correlation_dig}), given in the previous chapter, is used as the main operation of the correlation IQ System.



\subsubsubsection{Correlation by parts algorithm}

The correlation by parts algorithm is a method to efficiently compute the correlation between a received signal and the known parts of the preamble, considering the similarity with in-phase and quadrature tones.
\newp
At the beginning we have a window of size $N_w$, which is the size of the total preamble ($N_{symbols}$ multiplied by the size of each symbol $M$), plus the number of samples incoming in an earing cicle ($N_c$). With this structure, we allow only one instance of correlation where the incoming preamble is fully contained in the window. So from the window we can define $N_{symbols}$ parts, where each part $S_N$ corresponds to the section that could contain the $``N"th$ symbol of the preamble, it's lenght is $M+N_c$. In the figure \ref{fig:corr-by-parts-diagram} the upper array shows the window structure, every $S_n$ section is marked as $W_n$ with a different color.
\newp
once we have the $S_N$ and $P_N$ (the $``N"th$ symbol of the preamble) we can compute the correlation between them using the formula (\ref{eq:Correlation_dig}). That is implemented as the scipy function \textit{correlate} with mode ``valid", that has an output size of the difference between the two arguments plus one ($S_N - P_N + 1$), that is $N_c + 1$. For each of these $``N"$ calculus, the $S_N$ signals are correlated with its corresponding $P_N$ symbol and its version in quadrature, that means the following:
...
So at the output, we have two arrays that represent the similarity of the incoming signal section with the corresponding symbol in phase and quadrature. If we consider the totality of operations, we can define two matrices, I and Q, of size $N_{symbols}$ x $N_c + 1$, where each row corresponds to the arrays resulting in \ref{eq:corr_IQ}:
...
The matrices I and Q are represented in green and pink in the diagram of figure \ref{fig:corr-by-parts-diagram-2}. Then we take the absolute value of each element of the two matrices and sum them to obtain a final correlation matrix C, present in the lower part of the same diagram.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.55\textwidth]{img/Simulacion corr IQ.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{img/diagrams/diagrama_correlacion_por_partes2.png}
    \caption{Diagram of the correlation by parts process.}
    \label{fig:corr-by-parts-diagram-2}
\end{figure}
...
\begin{figure}
    \centering
    \includegraphics[width=0.95\textwidth]{img/Simulacion corr modelos_2.png}
...
The structure of the transmitted message consists of a preamble that have it's own symbol quantity and length, followed by the data symbols that represent the actual message to be transmitted and parity symbols based on Reed-Solomon error correction code, which are added at the end of the message. 

\begin{figure}
    \centering
    \includegraphics[width=0.8\textwidth]{img/diagrams/diagramamensaje.png}
    \caption{Structure of the transmitted message.}
    \label{fig:message-structure}
\end{figure}


The preamble is designed to facilitate synchronization and detection, while the data symbols carry the information content of the message. The parity symbols provide redundancy that allows for error detection and correction at the receiver, improving the reliability of the communication link in noisy environments.
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\textwidth]{img/decimacion/filters_decimation.png}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.49\textwidth}
        ...
        \caption{Correlation from Signal 64 symbols without decimation.}
        \label{fig:64_NW}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.49\textwidth}
        ...
        \caption{Correlation from Signal 64 symbols with decimation.}
        \label{fig:64_NW_D}
    \end{subfigure}
    \caption{Correlation from Signal with and without decimation for signals with 64 symbols.}
    \label{fig:corr-peak-variability-64}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.49\textwidth}
        ...
        \caption{Correlation from Signal 128 symbols without decimation.}
        \label{fig:128_NW}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.49\textwidth}
        ...
        \caption{Correlation from Signal 128 symbols with decimation.}
        \label{fig:128_NW_D}
    \end{subfigure}
    \caption{Correlation from Signal with and without decimation for signals with 128 symbols.}
    \label{fig:corr-peak-variability-128}
\end{figure}



In principle the form of the simple correlation is similar in both cases, however, when analyzing the variability of the peak of correlation over multiple measurements, it is observed that when the data is decimated, the variability increase:
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/decimacion/Power_and_xcorr_64_nw_1.png}
        \caption{Variability of correlation at model signal 1.}
        \label{fig:correlation-peak-variability-no-decimation}
    \end{subfigure}
    \caption{Variability of correlation peak at model signal 1.}
    \label{fig:correlation-peak-variability-graph-1}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/decimacion/Power_and_xcorr_128_nw_1.png}
        \caption{Variability of correlation peak at model signal 2.}
        \label{fig:correlation-peak-variability-decimation}
    \end{subfigure}
    \caption{Variability of correlation peak at model signal 2.}
    \label{fig:correlation-peak-variability-graph-2}
\end{figure}
...
\subsection{Induction Pattern}
 The decrement of the power available in the receiver coil as a function of the relative angle between coils was measured at the surface level in an open field. The transmitter coil was fixed in a horizontal position, while the receiver coil was rotated in steps of 5$\degree$ 
...
The resulting induction pattern is shown in the figure below:

 \begin{figure}
    \centering
    \includegraphics[width=0.7\textwidth]{img/patron.jpg}
    \caption{Resulting induction pattern (horizontal cut).}
    \label{fig:induction-pattern-setup}
 \end{figure}

As expected, the power received is maximum when the coils are perfectly aligned (0$\degree$) and decreases as the angle of missalignment increases. The decrement of voltage induced in the receiver coil follows a cosine pattern, which is consistent with the theoretical model of angular misalignment \ref{equation_of_flux_density_angular_misalignment}. In this case, the power delivered is proportional to the square of the voltage, so the decrement of power follows a cosine squared pattern. At 45$\degree$, the power received is approximately 50\% of the maximum value (-3 dB).
\newp
...
\begin{itemize}
    \item \textbf{Gautier-Copa de Agua} This corresponds to the shorter communication line, with an approximate distance of 100 meters between the transmitter and receiver. One of the units will be located in the basement of one of the observatory's optical telescopes, at an approximate depth of 3 meters. The other node will be located in the basement of an old water tower, at an approximate depth of 20 meters underground.
    
    \begin{figure}
        \centering
        \includegraphics[width=0.5\textwidth]{img/100m 200m and noise/100m.png}
    ...
    \end{figure}
        ...
        \begin{figure}
            \centering
            \includegraphics[width=0.47\textwidth]{img/100m 200m and noise/200m.png}
...
When the correlation by parts algorithm was implemented, it was necessary to validate its performance against the simple correlation method previously used in the system. For this purpose, the 100 m communication line was used to provide a controlled environment for comparison.
\newp
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{Comparison between Simple Correlation and Correlation by parts at 100m communication line. RX Copa de Agua.}
    \label{fig:correlation-methods-comparison-copa}
\end{figure}
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{Comparison between Simple Correlation and Correlation by parts at 100m communication line. RX Gautier.}
    \label{fig:correlation-methods-comparison-gau}
\end{figure}




The performance in terms of correlation peak variability achieved with both methods is summarized in the following graphs:
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{Performance comparison between Simple Correlation and Correlation by parts at 100m communication line. RX Copa de Agua.}
    \label{fig:perf-comp_100m_copa}
\end{figure}
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{Performance comparison between Simple Correlation and Correlation by parts at 100m communication line. RX Gautier.}
    \label{fig:perf-comp_100m_gau}
\end{figure}
...
The results obtained from both preamble models are summarized in the following graphs for the receiver at both locations (Gautier and Calicata):
...
The performance in terms of correlation peak variability achieved and Symbol Error Rate (SER) is summarized in the following graphs:
...
\newp
Here is a table of the Symbol Error Rate (SER) obtained in each test, considering both preamble models and the application of Reed Solomon error correction code with 20 parity symbols:

\begin{table}[H]
    \centering
    \begin{tabular}{|c|c|c|c|c|}
    \hline
    \textbf{Preamble Model} & \textbf{RX Loc.} & \textbf{SNR (dB)} &    \textbf{SER without RS} & \textbf{SER with RS} \\ \hline 
    12 X 48000 & Calicata & -10.5 &  2.25\%   &   1.25\% \\ \hline
    24 X 24000 & Calicata & -10.5 & 6.25\%    &   5.5\%   \\ \hline
    12 X 48000 & Gautier  & -8.04 dB & 8.25\% & 8\%        \\ \hline
    24 X 24000 & Gautier  & -8.04 dB & 5.55\% & 4.45\%     \\ \hline
\end{tabular}
\caption{Symbol Error Rate comparison between preamble models with and without Reed Solomon error correction.}
    \label{tab:SER_200M}
\end{table}

A video of a test conducted at the 200m communication line can be found in the link \cite{googleVideo_funcionamiento_MAGICmp4}.


\subsubsection{Operational test in a real mine}

Due to the good performance obtained in the previous test, the system was tested in an operational scenario, which means a real mine. MAGIC system was tested in the "Mina Benjamin", part of the mining company "José Iván Rojas Virraroel" located in the V region of Chile.
...
\end{figure}
\newp
The system was tested in four different communication lines, with distances between 15 and 120 meters, proving its capability horizontally and vertically. The test was the following:

\begin{itemize}
    \item Line of 15m: The Transmitter was located at the entrance of the mine, and the receiver was at a line of sight at 15 meters. This test was made to prove the basic operation of the system after the trip.
    \newp
 As a result, the system was able to communicate with a SER of 0\% and an SNR of 48 dB.
    ...
    \item Line of 14m: The transmitter was located at a certain mining mantle, and the receiver was in another mining mantle at a distance of 40m directly through rock. This was already a valuable measurement because these two mantles aren't communicated. As a result, the system was able to communicate with a SER of 0\% and an SNR of 26 dB.
    ...
    \item Line of 20m: The transmitter was located at a tunnel in the superior level and the receiver in a tunnel in a deep level, with a distance of 20m. This was the first vertical communication test. As a result, the system was able to communicate with a SER of 0\% and an SNR of 37 dB.
    ...
    \item Line of 120m: The transmitter was located deep in the mine at surface level, and the receiver was located outside the mine, near the offices, at a distance of 120m. This was the longest communication test. As a result, the system was able to communicate with an average SER of 0.35\%
...
The development and testing of the TTE communication system, MAGIC, have yielded promising results that demonstrate its potential for reliable underground communication. The system's design, which incorporates magnetic induction technology, a functional software based on a 16 FSK model, and error correction coding, has been validated through a series of field tests in various underground environments.

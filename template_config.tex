...
% NOMBRE DE OBJETOS
\def\nameabstract {Abstract}          % Name of abstract
\def\nameagradec {Acknowledgments}    % Name of acknowledgments chapter
\def\nameappendixsection {Appendices} % Name of appendix section
\def\namechapter {Chapter}            % Name of chapters
\def\nameltappendixsection {Appendix} % Appendix label
\def\nameltcont{Table of Contents}    % Name of contents index
\def\namelteqn {List of Equations}    % Name of equations list
\def\nameltfigure{List of Figures}    % Name of figures list
\def\nameltsrc {List of Listings}     % Name of source-code list
\def\namelttable {List of Tables}     % Name of tables list
\def\nameltwfigure {Figure}           % Figure caption label
\def\nameltwsrc {Listing}             % Source-code caption label
\def\nameltwtable {Table}             % Table caption label
\def\namemathcol {Corollary}          % Corollary name
\def\namemathdefn {Definition}        % Definition name
\def\namemathej {Example}             % Example name
\def\namemathlem {Lemma}              % Lemma name
\def\namemathobs {Observation}        % Observation name
\def\namemathprp {Proposition}        % Proposition name
\def\namemaththeorem {Theorem}        % Theorem name
\def\namepageof { of }                % Page label # of #
\def\nameportraitpage {Cover}         % Etiqueta página de la portada
\def\namereferences {References} % Name of references section
